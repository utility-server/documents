<p align="center">
  <img src="https://customer-assets.emergentagent.com/job_api-legal-hub/artifacts/1fjsrc03_logo-4X.png" alt="Utility Server Logo" width="200"/>
</p>

# Legal Documents

**DNS Stack Private Limited - Utility Server Legal & Corporate Documentation**

**Company**: DNS Stack Private Limited  
**CIN**: U62099UP2025PTC223463  
**Product Brand**: Utility Server (SPIDER, RHINO, MARLIN)  
**Website**: https://www.dnsstack.com

---

## Overview

This folder contains all legal policies, agreements, and compliance documentation for Utility Server's web performance optimization services, provided by DNS Stack Private Limited.

**Utility Server** is a product of **DNS Stack Private Limited**, an Indian company specializing in AI-powered web performance optimization tools.

**Subsidiary**: Techtonic Systems Media and Research LLC (https://techtonic.systems) is a subsidiary of DNS Stack Private Limited.

---

## ⚠️ IMPORTANT: Automatic Agreement

**BY USING UTILITY SERVER SERVICES, YOU AUTOMATICALLY AGREE TO ALL DOCUMENTS IN THIS REPOSITORY:**

✅ Terms and Conditions  
✅ Privacy Policy  
✅ Service Level Agreement (SLA)  
✅ Disclaimer  
✅ Non-Disclosure Agreement (NDA)  
✅ All other policies referenced herein

**Your registration for or use of Utility Server services constitutes your acceptance of these agreements. No separate signature is required.**

---

## 🔒 Data Processing Notice

**Utility Server processes ONLY publicly available data from your website.**

When you onboard to Utility Server services, you explicitly consent to:
- Access and processing of your public website content
- Caching and optimization of publicly accessible pages
- Analysis for performance improvements
- Processing necessary to deliver our services

**We do NOT access:**
- Password-protected content
- Private user data
- Non-public information
- Any data beyond what is publicly available on your website
- **Payment card information** (handled by payment gateway partners)

---

## 💳 Payment Processing

**⚠️ NO PAYMENT DATA ON OUR SERVERS:**

Utility Server and DNS Stack Private Limited **DO NOT** process, store, or have access to any payment information. All payment processing is handled by:
- Stripe (PCI-DSS Level 1 compliant)
- Other authorized payment gateway partners

Your payment information goes directly to the payment gateway and is never stored on our servers.

---

## 🤝 Channel Partner & White Label Model

**⚠️ IMPORTANT - DISTRIBUTION MODEL:**

Utility Server is a **white label solution** that may be sold through authorized channel partners (resellers, distributors, system integrators).

**If you purchased through a channel partner**:
- You are subject to both our terms AND your channel partner's terms
- Billing and contracts are handled by your channel partner
- Service delivery is provided by DNS Stack Private Limited

**We are NOT responsible for**:
- Channel partners' business practices or pricing
- Channel partners' client relationships
- Channel partners' support or service delivery processes
- Disputes between channel partners and their clients

---

## 📚 Available Documents

### Corporate Philosophy

#### [PHILOSOPHY.md](https://documents.utility-server.com/PHILOSOPHY.html)
**Our Mission, Vision, Values & Principles**
- Company purpose and mission statement
- 7 core values with detailed explanations
- Product philosophy (SPIDER, RHINO, MARLIN)
- Commitments to customers, team, industry, and planet
- Decision-making framework
- Guiding principles

---

### Customer Agreements

#### [TERMS_AND_CONDITIONS.md](https://documents.utility-server.com/TERMS_AND_CONDITIONS.html)
**Service Agreement & Terms of Use**
- 20 comprehensive sections
- Account registration and security
- Service plans and pricing (all tiers)
- Usage limits and fair use policy
- Payment terms, refunds, and billing
- Intellectual property rights
- User obligations and prohibited activities
- Warranties, disclaimers, and limitations
- Indemnification and dispute resolution

#### [SLA.md](https://documents.utility-server.com/SLA.html)
**Service Level Agreement**
- Uptime commitments: 99.5% - 99.95% by tier
- Performance targets and response times
- Support service levels
- Service credit calculations
- Maintenance windows and exclusions
- Detailed calculation examples

---

### Privacy & Data Protection

#### [PRIVACY_POLICY.md](https://documents.utility-server.com/PRIVACY_POLICY.html)
**Comprehensive Privacy Policy**
- GDPR, CCPA, LGPD, PIPEDA compliant
- Information collection and usage
- Data sharing and protection practices
- User privacy rights
- International data transfers
- Cookie policy
- Regional-specific provisions:
  - European Union (GDPR)
  - United Kingdom (UK GDPR)
  - California (CCPA/CPRA)
  - Canada (PIPEDA)
  - Brazil (LGPD)
  - Australia, Japan, Singapore

---

### Legal Disclaimers

#### [DISCLAIMER.md](https://documents.utility-server.com/DISCLAIMER.html)
**Limitations of Liability & Warranties**
- 15 comprehensive sections
- "AS IS" and "AS AVAILABLE" statements
- No warranty of service
- Performance and results disclaimers
- Security limitations
- Data loss disclaimers
- Third-party service disclaimers
- Beta features disclaimers
- Complete limitation of liability
- User responsibilities

---

### Business Agreements

#### [NDA.md](https://documents.utility-server.com/NDA.html)
**Non-Disclosure Agreement Template**
- Mutual confidentiality agreement
- For partners, vendors, and business relationships
- Comprehensive confidentiality obligations
- 5-year confidentiality term
- Trade secret protections
- Intellectual property safeguards
- Injunctive relief provisions
- Ready for execution

---

### Compliance & Regulatory

#### [LEGAL.md](https://documents.utility-server.com/LEGAL.html)
**Legal Compliance Information**
- Company registration and tax information
- Intellectual property portfolio:
  - Trademarks (SPIDER™, RHINO™, MARLIN™)
  - Copyrights and patents
  - Trade secrets protection
- Security certifications:
  - SOC 2 Type II
  - ISO 27001
  - PCI DSS compliance
- Regulatory compliance:
  - US: CCPA, CPRA, federal regulations
  - EU: GDPR, ePrivacy, DSA
  - UK: UK GDPR
  - Canada: PIPEDA, CASL
  - Brazil: LGPD
  - Australia, Japan, Singapore
- Export control compliance
- Accessibility commitments (WCAG 2.1 Level AA)
- DMCA procedures
- Legal request handling

#### [TECHTONIC_SYSTEMS_US_LEGAL.md](https://documents.utility-server.com/TECHTONIC_SYSTEMS_US_LEGAL.html)
**Techtonic Systems Media and Research LLC - US Legal Information**
- Complete company information and formation details
- Corporate structure (subsidiary of DNS Stack Private Limited)
- **Legal jurisdiction**: All matters handled by DNS Stack in India
- US LLC compliance (Wyoming)
- EU/GDPR compliance framework
- International operations and global compliance
- Channel partner and white-label disclaimers
- Payment processing disclaimers
- Intellectual property ownership
- Multi-jurisdictional legal framework
- Contact information for legal matters

---

## 🎯 Document Purpose Guide

### For Customers
**Read Before Using Services:**
1. [TERMS_AND_CONDITIONS.md](https://documents.utility-server.com/TERMS_AND_CONDITIONS.html) - Your agreement with us
2. [PRIVACY_POLICY.md](https://documents.utility-server.com/PRIVACY_POLICY.html) - How we handle your data
3. [SLA.md](https://documents.utility-server.com/SLA.html) - Our service commitments
4. [DISCLAIMER.md](https://documents.utility-server.com/DISCLAIMER.html) - Important limitations

### For Partners & Vendors
**Business Relationships:**
1. [NDA.md](https://documents.utility-server.com/NDA.html) - Confidentiality agreement
2. [LEGAL.md](https://documents.utility-server.com/LEGAL.html) - Company and IP information
3. [TERMS_AND_CONDITIONS.md](https://documents.utility-server.com/TERMS_AND_CONDITIONS.html) - Business terms

### For Legal & Compliance Teams
**Due Diligence:**
1. [LEGAL.md](https://documents.utility-server.com/LEGAL.html) - Compliance certifications
2. [TECHTONIC_SYSTEMS_US_LEGAL.md](https://documents.utility-server.com/TECHTONIC_SYSTEMS_US_LEGAL.html) - US subsidiary legal information
3. [PRIVACY_POLICY.md](https://documents.utility-server.com/PRIVACY_POLICY.html) - Data protection practices
4. [TERMS_AND_CONDITIONS.md](https://documents.utility-server.com/TERMS_AND_CONDITIONS.html) - Legal framework
5. [SLA.md](https://documents.utility-server.com/SLA.html) - Service commitments

### For Regulators
**Regulatory Review:**
1. [LEGAL.md](https://documents.utility-server.com/LEGAL.html) - Compliance overview
2. [TECHTONIC_SYSTEMS_US_LEGAL.md](https://documents.utility-server.com/TECHTONIC_SYSTEMS_US_LEGAL.html) - US operations and jurisdiction
3. [PRIVACY_POLICY.md](https://documents.utility-server.com/PRIVACY_POLICY.html) - Privacy practices
4. [TERMS_AND_CONDITIONS.md](https://documents.utility-server.com/TERMS_AND_CONDITIONS.html) - User agreements

---

## 📧 Legal Contacts

### General Legal Inquiries
**Email**: legal@utility-server.com

### Specific Legal Matters

| Matter | Contact |
|--------|--------|
| Privacy & Data Protection | privacy@utility-server.com |
| DMCA / Copyright | dmca@utility-server.com |
| Security Incidents | security@utility-server.com |
| Compliance | compliance@utility-server.com |
| NDA Requests | nda@utility-server.com |
| Legal Requests / Subpoenas | legal-requests@utility-server.com |
| Data Protection Officer (GDPR) | dpo@utility-server.com |
| Abuse Reports | abuse@utility-server.com |

### Contact Address
```
DNS Stack Private Limited
Utility Server - Legal Department
2nd Floor, Kalkat Bhawan
Punjab Innovation Mission, Gmada Aerocity
SAS Nagar, Mohali, Punjab - 140306
India

CIN: U62099UP2025PTC223463
Website: https://www.dnsstack.com
Subsidiary: Techtonic Systems Media and Research LLC (https://techtonic.systems)
```

---

## 🌍 Regional Data Protection Officers

**European Union**: dpo@utility-server.com  
**United Kingdom**: dpo@utility-server.com  
**Brazil (LGPD)**: dpo@utility-server.com

---

## ✅ Compliance Overview

### Privacy Regulations
- ✅ GDPR (European Union)
- ✅ UK GDPR (United Kingdom)
- ✅ CCPA/CPRA (California, USA)
- ✅ PIPEDA (Canada)
- ✅ LGPD (Brazil)
- ✅ Privacy Act (Australia)
- ✅ APPI (Japan)
- ✅ PDPA (Singapore)

### Security Standards
- ✅ SOC 2 Type II
- ✅ ISO/IEC 27001
- ✅ PCI DSS (via Stripe)
- ✅ OWASP Top 10
- ✅ NIST Framework

### Accessibility
- ✅ WCAG 2.1 Level AA
- ✅ Section 508 (US)
- ✅ ADA (US)
- ✅ European Accessibility Act

---

## 🔄 Document Updates

### Version Control
All documents are version-controlled via Git. View commit history for changes.

### Effective Dates
Each document includes:
- Effective date
- Last updated date
- Version number (where applicable)

### Notification of Changes
Material changes are communicated via:
- Email to account holders (30 days notice)
- Website announcements
- In-service notifications
- Git commit log

---

## 📖 How to Use These Documents

### Reading Order for New Users
1. Start with [PHILOSOPHY.md](./PHILOSOPHY.html) to understand our values
2. Read [TERMS_AND_CONDITIONS.md](https://documents.utility-server.com/TERMS_AND_CONDITIONS.html) before using services
3. Review [PRIVACY_POLICY.md](https://documents.utility-server.com/PRIVACY_POLICY.html) to understand data practices
4. Check [SLA.md](https://documents.utility-server.com/SLA.html) for service commitments
5. Read [DISCLAIMER.md](https://documents.utility-server.com/DISCLAIMER.html) for important limitations

### For US-Based Users or Partners
1. [TECHTONIC_SYSTEMS_US_LEGAL.md](https://documents.utility-server.com/TECHTONIC_SYSTEMS_US_LEGAL.html) - US operations and legal framework
2. Understand that all legal matters are handled in India by DNS Stack

### For Legal Review
1. [LEGAL.md](https://documents.utility-server.com/LEGAL.html) - Compliance overview
2. [TECHTONIC_SYSTEMS_US_LEGAL.md](https://documents.utility-server.com/TECHTONIC_SYSTEMS_US_LEGAL.html) - US subsidiary information
3. [TERMS_AND_CONDITIONS.md](https://documents.utility-server.com/TERMS_AND_CONDITIONS.html) - Legal framework
4. [PRIVACY_POLICY.md](https://documents.utility-server.com/PRIVACY_POLICY.html) - Data protection
5. [SLA.md](https://documents.utility-server.com/SLA.html) - Service commitments
6. [NDA.md](https://documents.utility-server.com/NDA.html) - Confidentiality agreements

---

## ⚖️ Dispute Resolution

### Informal Resolution (Preferred)
1. Contact support: support@utility-server.com
2. Escalate to legal if needed: legal@utility-server.com
3. Work in good faith to resolve issues

### Formal Disputes
See [TERMS_AND_CONDITIONS.md](https://documents.utility-server.com/TERMS_AND_CONDITIONS.html) Section 19 for:
- Governing law
- Arbitration procedures
- Jurisdiction and venue
- Class action waiver

---

## 🔒 Security & Confidentiality

### Reporting Security Issues
**Email**: security@utility-server.com  
**Do NOT** report via public channels

### Requesting an NDA
**Email**: nda@utility-server.com  
**Template**: [NDA.md](https://documents.utility-server.com/NDA.html)

### Legal Requests
**Email**: legal-requests@utility-server.com  
**Process**: See [LEGAL.md](https://documents.utility-server.com/LEGAL.html) Section 11

---

## 🌐 International Users

### Data Protection Rights
See [PRIVACY_POLICY.md](https://documents.utility-server.com/PRIVACY_POLICY.html) Section 14 for regional-specific provisions:
- EU/EEA residents (GDPR)
- UK residents (UK GDPR)
- California residents (CCPA/CPRA)
- Canadian residents (PIPEDA)
- Brazilian residents (LGPD)
- Australian residents (Privacy Act)

### Export Control
See [LEGAL.md](https://documents.utility-server.com/LEGAL.html) Section 6 for:
- Export compliance
- Restricted countries
- User obligations

---

## 📊 Transparency

We believe in transparency:
- ✅ All legal documents publicly accessible
- ✅ Changes tracked via Git
- ✅ Clear contact information
- ✅ Responsive to inquiries
- ✅ Transparency reports (when volume permits)

---

## 🤝 Feedback Welcome

We welcome feedback on our legal documents:
- **Typos or errors**: Open a GitHub issue
- **Suggestions**: Email legal@utility-server.com
- **Legal questions**: Email appropriate contact above
- **Privacy concerns**: Email privacy@utility-server.com

---

## 📅 Last Updated

**Repository**: November 6, 2025  
**Individual Documents**: See each document for specific dates

---

## 🔗 Related Resources

- **Main Repository**: https://github.com/utility-server/documents
- **API Documentation**: [https://apis.utility-server.com/](https://apis.utility-server.com/)
- **Service Websites**:
  - SPIDER: https://spider.utility-server.com
  - RHINO: https://rhino.utility-server.com
  - MARLIN: https://marlin.utility-server.com
- **Status Page**: https://status.utility-server.com
- **Support**: support@utility-server.com

---

*For questions about these documents or our legal policies, contact legal@utility-server.com*

*© 2024-2025 DNS Stack Private Limited. All rights reserved.
Utility Server is a product of DNS Stack Private Limited.*