<p align="center">
  <img src="https://customer-assets.emergentagent.com/job_api-legal-hub/artifacts/1fjsrc03_logo-4X.png" alt="Utility Server Logo" width="200"/>
</p>

---

**Navigation:** [🏠 Legal Home](https://documents.utility-server.com/) | [📜 Philosophy](https://documents.utility-server.com/PHILOSOPHY.html) | [📋 Terms](https://documents.utility-server.com/TERMS_AND_CONDITIONS.html) | [🔒 Privacy](https://documents.utility-server.com/PRIVACY_POLICY.html) | [⚠️ Disclaimer](https://documents.utility-server.com/DISCLAIMER.html) | [🤝 NDA](https://documents.utility-server.com/NDA.html) | [⚖️ Legal](https://documents.utility-server.com/LEGAL.html) | [📊 SLA](https://documents.utility-server.com/SLA.html)

---

# Legal Information

**Utility Server Legal Compliance and Intellectual Property**

**Last Updated**: November 6, 2025

---

## Table of Contents

1. [Company Information](#1-company-information)
2. [Intellectual Property](#2-intellectual-property)
3. [Compliance and Certifications](#3-compliance-and-certifications)
4. [Regulatory Compliance](#4-regulatory-compliance)
5. [Data Protection and Privacy](#5-data-protection-and-privacy)
6. [Export Control](#6-export-control)
7. [Accessibility](#7-accessibility)
8. [Industry Standards](#8-industry-standards)
9. [Legal Policies](#9-legal-policies)
10. [Dispute Resolution](#10-dispute-resolution)
11. [Subpoenas and Legal Requests](#11-subpoenas-and-legal-requests)
12. [Contact Legal Team](#12-contact-legal-team)

---

## 1. Company Information

### 1.1 Legal Entity

**Company Name**: DNS Stack Private Limited

**Product/Service Brand**: Utility Server (SPIDER, RHINO, MARLIN)

**Business Type**: Private Limited Company

**Jurisdiction of Incorporation**: India

**Corporate Identification Number (CIN)**: U62099UP2025PTC223463

**Date of Incorporation**: May 8, 2025

**Company Status**: Active

**Registered Office Address**:
```
DNS Stack Private Limited
117/N/270, Tulsi Nagar
Naveen Nagar, Kanpur Nagar
Uttar Pradesh - 208025
India
```

**Principal Office Address**:
```
DNS Stack Private Limited
2nd Floor, Kalkat Bhawan
Punjab Innovation Mission, Gmada Aerocity
SAS Nagar, Mohali, Punjab - 140306
India
```

**Websites**: 
- https://www.dnsstack.com (Primary - DNS Stack Private Limited)
- https://techtonic.systems (Subsidiary - Techtonic Systems Media and Research LLC)

**Subsidiary Companies**:
- **Techtonic Systems Media and Research LLC**
  - Website: https://techtonic.systems
  - Formation Date: January 22, 2024
  - EIN: 61-2147769
  - Type: Limited Liability Company (LLC)
  - Jurisdiction: Texas, United States
  - Address: 701 Tillery St, Unit 12, Austin, TX 78702, USA
  - Business: Media and research subsidiary of DNS Stack Private Limited
  - Sole Member: Rajneesh Maurya

### 1.2 Tax Information (DNS Stack Private Limited)

**Corporate Identification Number (CIN)**: U62099UP2025PTC223463

**PAN (Permanent Account Number)**: AALCD6871C

**GST Registration Number**: 03AALCD6871C1ZR

**Jurisdictions where registered for tax**: India

**Industry Classification**: Computer Programming, Consultancy and Related Activities (NIC Code: 62099)

### 1.3 Subsidiary Tax Information (Techtonic Systems Media and Research LLC)

**Company Name**: Techtonic Systems Media and Research LLC

**Formation Date**: January 22, 2024

**EIN (Employer Identification Number)**: 61-2147769

**Type**: Limited Liability Company (LLC)

**Jurisdiction**: United States (Texas)

**Registered Address**: 701 Tillery St, Unit 12, Austin, TX 78702, USA

**Business Type**: Media and Research

**Sole Member**: Rajneesh Maurya

### 1.3 Business Licenses

Utility Server maintains all necessary business licenses and permits required to operate in its jurisdictions, including:
- Business operation licenses
- Professional services licenses
- Technology service provider registrations
- International business registrations (where applicable)

### 1.4 Business Model - White Label & Channel Partners

**Distribution Model**: Utility Server operates as a **white label solution** distributed through:
1. Direct sales by DNS Stack Private Limited
2. Authorized channel partners (resellers, distributors, system integrators)

**Channel Partner Framework**:
- Channel partners are authorized to resell and rebrand Utility Server services
- Channel partners must comply with our service standards and these legal documents
- Channel partners operate independently in their client relationships

**⚠️ LIMITATION OF RESPONSIBILITY:**

**DNS Stack Private Limited is NOT responsible for**:
- Channel partners' business operations, pricing, or billing practices
- Channel partners' client relationships and service delivery processes
- Channel partners' compliance with their local regulations
- Channel partners' data handling outside our service scope
- Channel partners' marketing, sales, or support practices
- Disputes between channel partners and their clients

**Service Delivery**: The core Utility Server technology and service is provided by DNS Stack Private Limited regardless of the sales channel.

### 1.5 Payment Processing

**⚠️ NO PAYMENT INFORMATION PROCESSED OR STORED:**

**DNS Stack Private Limited and Utility Server DO NOT**:
- Process payment transactions on our servers
- Store credit card or banking information
- Have access to complete payment credentials

**Payment Gateway Partners**:
All payment processing is handled exclusively by:
- Stripe (PCI-DSS Level 1 compliant)
- Other authorized PCI-compliant payment gateway partners

**What We Receive**:
- Transaction confirmation (success/failure)
- Transaction ID for reference
- Basic billing information (amount, date)
- NO payment card details or banking credentials

---

## 2. Intellectual Property

### 2.1 Brand Names and Service Marks

**Brand Names** (Unregistered - Common Law Rights):

- **DNS STACK** - Parent company brand
- **UTILITY SERVER** - Product brand
- **SPIDER** - Prerendering service name
- **RHINO** - Edge caching service name
- **MARLIN** - Image optimization service name
- **Utility Client** - CLI tool name
- **Techtonic Systems** - Subsidiary brand

**Status**: These are proprietary brand names and service identifiers owned by DNS Stack Private Limited and its subsidiary. While not currently registered as trademarks, they are protected under common law and business name registrations.

**Notice**: All brand names, service names, and logos used on our website and in connection with our services are proprietary to DNS Stack Private Limited or its subsidiary, Techtonic Systems Media and Research LLC. Unauthorized use is strictly prohibited.

### 2.2 Copyrights

**Copyright Notice**:
© 2024-2025 DNS Stack Private Limited. All rights reserved.

Utility Server is a product of DNS Stack Private Limited.

**Protected Works**:
- Software and source code
- Documentation and manuals
- Website content and design
- Marketing materials
- Educational content and tutorials
- API documentation
- User interfaces and design elements
- Logos, graphics, and visual assets

**Copyright Registration**: Selected works are registered with relevant copyright offices.

### 2.3 Proprietary Technology

DNS Stack Private Limited and its Utility Server product line utilize proprietary technology and methodologies. While no patents are currently filed or registered, the company reserves all rights to its technological innovations, algorithms, and business methods.

**Note**: DNS Stack Private Limited may file patent applications for novel technologies in the future as the business and technology evolve.

### 2.4 Trade Secrets

Utility Server protects proprietary information as trade secrets, including:
- Algorithms and methodologies
- System architectures
- Performance optimization techniques
- Business processes and strategies
- Customer data and analytics

**Protection Measures**:
- Confidentiality agreements with employees and contractors
- Access controls and security measures
- Non-disclosure agreements with partners
- Legal enforcement of trade secret rights

### 2.5 Open Source Software

Utility Server respects open source licenses and maintains compliance with:
- All applicable open source licenses (MIT, Apache 2.0, GPL, etc.)
- Attribution requirements
- License compatibility
- Distribution obligations

**Open Source Compliance**:
- We maintain an inventory of open source components
- We provide required notices and attributions
- We comply with copyleft requirements where applicable
- We contribute back to open source communities

### 2.6 Third-Party IP

We respect third-party intellectual property rights:
- We obtain necessary licenses for third-party software and services
- We comply with attribution and notice requirements
- We respond promptly to IP infringement claims
- We have procedures for DMCA takedown notices

### 2.7 Licensing Our IP

For licensing inquiries regarding Utility Server intellectual property:
- Email: licensing@utility-server.com
- We consider licensing arrangements for qualified partners
- Terms are negotiated on a case-by-case basis

---

## 3. Compliance and Certifications

### 3.1 Security Certifications

**SOC 2 Type II**:
- Status: [Certified / In Progress]
- Scope: Security, Availability, Confidentiality
- Audit Firm: [Name of auditing firm]
- Last Audit: [Date]
- Next Audit: [Date]

**ISO 27001** (if applicable):
- Status: [Certified / In Progress]
- Certificate Number: [Number]
- Certification Body: [Name]
- Valid Until: [Date]

**PCI DSS** (if applicable):
- Level: [Level 1, 2, 3, or 4]
- Compliance validated through: [QSA / SAQ]
- Last validation: [Date]
- Note: Payment processing handled by PCI-compliant third parties (Stripe)

### 3.2 Privacy Certifications

**Privacy Shield** (if applicable):
- Note: Privacy Shield framework was invalidated; we use Standard Contractual Clauses

**Privacy Seal Programs**:
- [List any privacy seal programs]

### 3.3 Industry Certifications

**Cloud Security Alliance (CSA)**:
- STAR Certification (if applicable)

**Uptime Institute**:
- Tier Certification for data centers (if applicable)

### 3.4 Compliance Reports

Available compliance reports:
- SOC 2 reports (available to customers under NDA)
- Security audit summaries
- Penetration test reports (summary)
- Compliance attestations

**Request Reports**: Contact compliance@utility-server.com

---

## 4. Regulatory Compliance

### 4.1 United States Compliance

**Federal Regulations**:
- Computer Fraud and Abuse Act (CFAA)
- Electronic Communications Privacy Act (ECPA)
- Digital Millennium Copyright Act (DMCA)
- CAN-SPAM Act
- Federal Trade Commission Act

**State Regulations**:

**Important Note**: Since Utility Server processes ONLY publicly available website data (not personal information of individual users), most consumer privacy laws such as CCPA/CPRA do not apply to our core service operations. These laws regulate the collection and processing of personal information of consumers, which we do not collect through our optimization services.

However, for customer account information (our direct customers), we comply with:
- Data security requirements
- Transparent privacy practices
- Account information protection

**Sector-Specific**: Not applicable - we do not process healthcare data (HIPAA), financial data (GLBA), educational records (FERPA), or children's data (COPPA).

### 4.2 European Union Compliance

**General Data Protection Regulation (GDPR)**:

**Important Note**: Utility Server processes ONLY publicly available data from websites. Since we do not collect, process, or store personal data of EU residents or any individual users (we only cache public web pages), most GDPR requirements related to personal data protection do not apply to our core operations.

However, for customer account information, we maintain:
- Lawful basis for processing: Contract, Legitimate Interest
- Data Protection Officer: dpo@utility-server.com
- Privacy by design and default
- Customer data security measures

**Public Data Processing**: As we only process public website content (HTML, CSS, JavaScript, images) that is already publicly accessible on the internet, this does not constitute personal data processing under GDPR Article 4.

**ePrivacy Directive**:
- Cookie consent mechanisms
- Electronic communications privacy
- Marketing communications consent

**Digital Services Act (DSA)** (if applicable):
- Compliance with DSA requirements for digital service providers

**Data Protection Authority (DPA) Registration**:
- [List relevant DPA registrations]

### 4.3 United Kingdom Compliance

**UK GDPR**:
- Similar requirements to EU GDPR
- ICO registration: [Registration number]
- UK Representative (if applicable): [Contact information]

### 4.4 Canadian Compliance

**Personal Information Protection and Electronic Documents Act (PIPEDA)**:
- Privacy principles compliance
- Breach notification procedures
- Individual rights procedures

**Anti-Spam Legislation (CASL)**:
- Consent mechanisms for commercial electronic messages
- Identification and unsubscribe mechanisms

### 4.5 Asia-Pacific Compliance

**Australia**:
- Privacy Act 1988
- Australian Privacy Principles (APPs)
- Notifiable Data Breaches scheme

**Singapore**:
- Personal Data Protection Act (PDPA)

**Japan**:
- Act on the Protection of Personal Information (APPI)

**Other Jurisdictions**:
- Compliance with local data protection and privacy laws as applicable

### 4.6 Latin America Compliance

**Brazil**:
- Lei Geral de Proteção de Dados (LGPD)
- Data Protection Officer: dpo@utility-server.com
- Brazilian representative (if applicable): [Contact information]

**Other Jurisdictions**:
- Mexico: Federal Law on Protection of Personal Data
- Argentina: Personal Data Protection Law
- Compliance with local regulations as applicable

### 4.7 Global Compliance

**International Standards**:
- ISO/IEC 27001 (Information Security Management)
- ISO/IEC 27018 (Cloud Privacy)
- ISO/IEC 27701 (Privacy Information Management)

**Industry Best Practices**:
- Cloud Security Alliance (CSA) guidelines
- NIST Cybersecurity Framework
- CIS Controls
- OWASP security practices

---

## 5. Data Protection and Privacy

### 5.1 Data Protection Principles

Utility Server adheres to internationally recognized data protection principles:

1. **Lawfulness, Fairness, and Transparency**: Process data lawfully and transparently
2. **Purpose Limitation**: Collect data for specified, legitimate purposes
3. **Data Minimization**: Collect only necessary data
4. **Accuracy**: Maintain accurate and up-to-date data
5. **Storage Limitation**: Retain data only as long as necessary
6. **Integrity and Confidentiality**: Ensure appropriate security
7. **Accountability**: Demonstrate compliance

### 5.2 Data Processing Agreements

We provide Data Processing Agreements (DPAs) that include:
- Standard Contractual Clauses (SCCs) for international transfers
- Processing instructions and limitations
- Security obligations
- Sub-processor provisions
- Data subject rights procedures
- Audit rights
- Breach notification procedures

**Request DPA**: Contact legal@utility-server.com

### 5.3 International Data Transfers

**Transfer Mechanisms**:
- Standard Contractual Clauses (EU and UK)
- Adequacy decisions (where applicable)
- Binding Corporate Rules (if applicable)
- Additional safeguards and encryption

**Data Localization**:
- Data may be stored in multiple jurisdictions
- Customers can specify preferred data locations (Enterprise plans)
- We comply with data residency requirements

### 5.4 Data Subject Rights

We facilitate the following data subject rights:
- Right of access
- Right to rectification
- Right to erasure ("right to be forgotten")
- Right to restriction of processing
- Right to data portability
- Right to object
- Rights related to automated decision-making

**Exercise Rights**: Contact privacy@utility-server.com

### 5.5 Breach Notification

In the event of a data breach:
- We have incident response procedures
- We will notify affected individuals as required by law (typically within 72 hours)
- We will notify relevant supervisory authorities
- We will provide information about the breach and remediation steps

**Report a Breach**: security@utility-server.com (for external reporting)

---

## 6. Export Control

### 6.1 Export Compliance

Utility Server complies with export control laws, including:

**United States**:
- Export Administration Regulations (EAR)
- International Traffic in Arms Regulations (ITAR) - if applicable
- Office of Foreign Assets Control (OFAC) sanctions

**European Union**:
- EU Dual-Use Regulation
- Member state export controls

**Other Jurisdictions**:
- Compliance with local export control laws

### 6.2 Restricted Countries

Services may not be available in countries subject to comprehensive sanctions, including:
- Countries on US embargo lists (Cuba, Iran, North Korea, Syria, Crimea region)
- Other sanctioned regions as updated by authorities

### 6.3 Prohibited Users

Services may not be used by:
- Individuals or entities on sanctions lists (SDN, Entity List, etc.)
- Parties involved in prohibited activities
- Users in restricted countries

### 6.4 Customer Obligations

Customers agree to:
- Comply with all applicable export control laws
- Not use Services in violation of export restrictions
- Not provide access to restricted parties
- Notify us if they become subject to restrictions

---

## 7. Accessibility

### 7.1 Accessibility Commitment

Utility Server is committed to making our services accessible to all users, including those with disabilities.

### 7.2 Standards Compliance

We strive to comply with:

**Web Content Accessibility Guidelines (WCAG) 2.1**:
- Target: Level AA compliance
- Ongoing testing and improvements

**Section 508** (US Rehabilitation Act):
- Compliance for US government customers

**Americans with Disabilities Act (ADA)**:
- Accessibility standards for digital services

**European Accessibility Act (EAA)**:
- Compliance with EU accessibility requirements

### 7.3 Accessibility Features

- Keyboard navigation
- Screen reader compatibility
- Alternative text for images
- Clear color contrast
- Resizable text
- Clear headings and structure

### 7.4 Ongoing Efforts

- Regular accessibility audits
- User testing with assistive technologies
- Employee training on accessibility
- Continuous improvement based on feedback

### 7.5 Accessibility Feedback

For accessibility concerns or requests:
- Email: accessibility@utility-server.com
- We welcome feedback and will address issues promptly

---

## 8. Industry Standards

### 8.1 Technical Standards

Utility Server adheres to industry technical standards:

**Web Standards**:
- W3C (World Wide Web Consortium) standards
- WHATWG (Web Hypertext Application Technology Working Group)
- HTML5, CSS3, ECMAScript specifications

**Internet Standards**:
- IETF (Internet Engineering Task Force) RFCs
- HTTP/1.1, HTTP/2, HTTP/3
- TLS 1.2, TLS 1.3
- DNS, DNSSEC

**Image Formats**:
- JPEG, PNG, GIF, WebP, AVIF
- Industry-standard compression and optimization

**Security Standards**:
- OWASP Top 10
- CWE/SANS Top 25
- NIST guidelines

### 8.2 Industry Memberships

**Professional Associations** (if applicable):
- Cloud Security Alliance (CSA)
- Internet Society (ISOC)
- Content Delivery & Security Association (CDSA)
- [Other relevant associations]

### 8.3 Best Practices

We follow industry best practices for:
- Software development lifecycle (SDLC)
- Secure coding practices
- DevSecOps
- Infrastructure as Code (IaC)
- Continuous integration/continuous deployment (CI/CD)
- Incident response
- Business continuity and disaster recovery

---

## 9. Legal Policies

### 9.1 Core Legal Documents

**Available at https://github.com/utility-server/documents**:

1. **Terms and Conditions**: Governing use of Services
2. **Privacy Policy**: Data collection and use practices
3. **Service Level Agreement (SLA)**: Service commitments and uptime
4. **Acceptable Use Policy**: Prohibited uses and content
5. **Cookie Policy**: Use of cookies and tracking technologies
6. **DMCA Policy**: Copyright infringement procedures
7. **Disclaimer**: Limitations of liability and warranties
8. **Non-Disclosure Agreement**: Confidentiality for partners

### 9.2 Acceptable Use Policy

Prohibited activities include:
- Illegal content or activities
- Copyright infringement
- Malware distribution
- Spamming or phishing
- Unauthorized access attempts
- Resource abuse
- Harmful or offensive content

Full policy: [Link to AUP]

### 9.3 DMCA and Copyright Policy

We respond to valid copyright infringement notices under the Digital Millennium Copyright Act (DMCA).

**Designated DMCA Agent**:
```
Utility Server
DMCA Compliance
[Address]
Email: dmca@utility-server.com
```

**Notice Requirements**:
- Identification of copyrighted work
- Identification of infringing material
- Contact information
- Good faith statement
- Statement of accuracy
- Physical or electronic signature

Full policy: [Link to DMCA Policy]

### 9.4 Subpoena and Legal Request Policy

We comply with valid legal requests for information, including:
- Court orders
- Subpoenas
- Search warrants
- Other lawful requests

**Our Process**:
- We review all requests for legal validity
- We notify affected customers when legally permissible
- We provide only the minimum information required
- We maintain transparency reports

**Submit Legal Requests**: legal-requests@utility-server.com

---

## 10. Dispute Resolution

### 10.1 Informal Resolution

We encourage informal resolution of disputes:
- Contact our support team first
- We will work in good faith to resolve issues
- Most disputes can be resolved amicably

### 10.2 Formal Dispute Resolution

For unresolved disputes:

**Governing Law**: [Insert Jurisdiction]

**Dispute Resolution Method**: 
- Binding arbitration (as specified in Terms and Conditions)
- Exception: Injunctive relief for IP infringement

**Arbitration Rules**: [Insert Arbitration Association and Rules]

**Venue**: [Insert City, State/Country]

### 10.3 Class Action Waiver

Disputes must be brought individually, not as class actions (as detailed in Terms and Conditions).

### 10.4 Small Claims Court

Either party may bring claims in small claims court if within jurisdiction and guidelines.

---

## 11. Subpoenas and Legal Requests

### 11.1 Law Enforcement Requests

We cooperate with law enforcement within legal bounds:

**Requirements for Requests**:
- Must be submitted by authorized law enforcement
- Must be properly formatted and signed
- Must specify the legal basis
- Must be reasonably specific

**How We Respond**:
- We review for legal sufficiency
- We may challenge overbroad requests
- We notify users when permitted
- We provide only required information

### 11.2 Civil Subpoenas

For civil litigation subpoenas:
- We require proper service
- We may object to unduly burdensome requests
- We charge reasonable fees for production
- We notify affected customers when possible

### 11.3 Emergency Requests

For emergencies involving imminent danger:
- Contact: emergency@utility-server.com
- We may provide information without legal process in good faith
- Must still provide follow-up legal documentation

### 11.4 Transparency

We believe in transparency:
- We publish annual transparency reports (when volume permits)
- Reports include types and numbers of requests
- We advocate for user privacy rights

### 11.5 International Requests

For international legal requests:
- We may require use of Mutual Legal Assistance Treaties (MLATs)
- We comply with local laws where we operate
- We assess conflicts of law situations carefully

---

## 12. Contact Legal Team

### 12.1 General Legal Inquiries

**Email**: legal@utility-server.com

**To DNS Stack Private Limited (Utility Server):**

**Contact Address**:
```
DNS Stack Private Limited
2nd Floor, Kalkat Bhawan
Punjab Innovation Mission, Gmada Aerocity
SAS Nagar, Mohali, Punjab - 140306
India

Email: legal@utility-server.com
Website: https://www.dnsstack.com
Subsidiary: Techtonic Systems Media and Research LLC (https://techtonic.systems)
```

*Note: Registered office address available in Section 1.1 above for statutory reference.*

### 12.2 Specific Legal Matters

| Matter | Contact |
|--------|--------|
| Privacy and Data Protection | privacy@utility-server.com |
| DMCA / Copyright | dmca@utility-server.com |
| Security Incidents | security@utility-server.com |
| Legal Requests / Subpoenas | legal-requests@utility-server.com |
| Compliance | compliance@utility-server.com |
| Licensing | licensing@utility-server.com |
| NDA Requests | nda@utility-server.com |
| Terms Violations | abuse@utility-server.com |

### 12.3 Data Protection Officers

**EU Data Protection Officer**:
- Email: dpo@utility-server.com

**UK Data Protection Officer**:
- Email: dpo-uk@utility-server.com (if separate)

**Brazil Data Protection Officer** (LGPD):
- Email: dpo-brazil@utility-server.com (if separate)

### 12.4 Representatives

**EU Representative** (if applicable per GDPR Article 27):
```
[Name]
[Address]
[Email]
```

**UK Representative** (if applicable):
```
[Name]
[Address]
[Email]
```

---

## 13. Updates to Legal Information

### 13.1 Changes

This legal information page may be updated periodically to reflect:
- Changes in laws and regulations
- New certifications or compliance achievements
- Organizational changes
- Policy updates

### 13.2 Material Changes

For material changes affecting customer rights or obligations:
- We will provide 30 days' notice
- Notice via email and website announcement
- Continued use constitutes acceptance

### 13.3 Review

We encourage periodic review of our legal policies and information.

---

## 14. Additional Resources

### 14.1 Legal Documents Repository

**GitHub**: https://github.com/utility-server/documents

All legal policies and documents are maintained in our public repository.

### 14.2 Security Information

**Security Page**: https://utility-server.com/security

**Bug Bounty Program** (if applicable): [Link]

### 14.3 Compliance Documentation

**Trust Center**: https://trust.utility-server.com (if applicable)

Access to:
- Compliance certifications
- Security documentation
- Privacy resources
- Audit reports (under NDA)

### 14.4 Industry Participation

**Blog**: https://blog.utility-server.com
- Legal and compliance updates
- Industry commentary
- Best practices

---

## Conclusion

Utility Server is committed to operating with the highest legal and ethical standards. We maintain compliance with applicable laws, respect intellectual property rights, and protect customer data.

For any legal questions, concerns, or requests, please contact our legal team using the information provided above.

---

*Last Updated: November 6, 2025*

*© 2024 Utility Server. All rights reserved.*

*This legal information is provided for transparency and informational purposes. For specific legal advice, consult qualified legal counsel.*